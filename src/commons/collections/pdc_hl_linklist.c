...
create_entry()
{
    list_entry_t *new_entry =
        (list_entry_t *)ctr_calloc(1, sizeof(list_entry_t), &mem_usage_by_all_linkedlist);
    /*
    if (!new_entry) {
...
{
    tagged_value_t *newval =
        (tagged_value_t *)ctr_calloc(1, sizeof(tagged_value_t), &mem_usage_by_all_linkedlist);
    if (!newval) {
