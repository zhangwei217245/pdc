...
ListEntry *
list_prepend(ListEntry **list, ListValue data)
{
    ListEntry *newentry;
...
ListEntry *
list_append(ListEntry **list, ListValue data)
{
    ListEntry *rover;
    ListEntry *newentry;
...
void
list_set_data(ListEntry *listentry, ListValue value)
{
    if (listentry != NULL) {
...
ListEntry *
list_nth_entry(ListEntry *list, unsigned int n)
{
    ListEntry *  entry;
    unsigned int i;
...
ListValue
list_nth_data(ListEntry *list, unsigned int n)
{
    ListEntry *entry;
...
int
list_remove_entry(ListEntry **list, ListEntry *entry)
{
...
unsigned int
list_remove_data(ListEntry **list, ListEqualFunc callback, ListValue data)
{
...
static ListEntry *
list_sort_internal(ListEntry **list, ListCompareFunc compare_func)
{
    ListEntry *pivot;
    ListEntry *rover;
...
void
list_sort(ListEntry **list, ListCompareFunc compare_func)
{
    list_sort_internal(list, compare_func);
}

ListEntry *
list_find_data(ListEntry *list, ListEqualFunc callback, ListValue data)
{
    ListEntry *rover;
...
void
list_iterate(ListEntry **list, ListIterator *iter)
{
...
int
list_iter_has_more(ListIterator *iter)
{
...
ListValue
list_iter_next(ListIterator *iter)
{
    if (iter->current == NULL || iter->current != *iter->prev_next) {
...
void
list_iter_remove(ListIterator *iter)
{
    if (iter->current == NULL || iter->current != *iter->prev_next) {
