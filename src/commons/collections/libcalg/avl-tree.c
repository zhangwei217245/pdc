...
AVLTree *
avl_tree_new(AVLTreeCompareFunc compare_func)
{
    AVLTree *new_tree;
...
static void
avl_tree_free_subtree(AVLTree *tree, AVLTreeNode *node)
{
    if (node == NULL) {
        return;
    }
...
int
avl_tree_subtree_height(AVLTreeNode *node)
{
    if (node == NULL) {
        return 0;
    }
...
static void
avl_tree_update_height(AVLTreeNode *node)
{
...
static AVLTreeNodeSide
avl_tree_node_parent_side(AVLTreeNode *node)
{
...
static void
avl_tree_node_replace(AVLTree *tree, AVLTreeNode *node1, AVLTreeNode *node2)
{
    int side;
...
static AVLTreeNode *
avl_tree_rotate(AVLTree *tree, AVLTreeNode *node, AVLTreeNodeSide direction)
{
...
static AVLTreeNode *
avl_tree_node_balance(AVLTree *tree, AVLTreeNode *node)
{
    AVLTreeNode *left_subtree;
    AVLTreeNode *right_subtree;
    AVLTreeNode *child;
...
static void
avl_tree_balance_to_root(AVLTree *tree, AVLTreeNode *node)
{
    AVLTreeNode *rover;
...
AVLTreeNode *
avl_tree_insert(AVLTree *tree, AVLTreeKey key, AVLTreeValue value)
{
    AVLTreeNode **rover;
...
static AVLTreeNode *
avl_tree_node_get_replacement(AVLTree *tree, AVLTreeNode *node)
{
    AVLTreeNode *left_subtree;
    AVLTreeNode *right_subtree;
    AVLTreeNode *result;
...
void
avl_tree_remove_node(AVLTree *tree, AVLTreeNode *node)
{
...
int
avl_tree_remove(AVLTree *tree, AVLTreeKey key)
{
    AVLTreeNode *node;
...
AVLTreeNode *
avl_tree_lookup_node(AVLTree *tree, AVLTreeKey key)
{
    AVLTreeNode *node;
    int          diff;
...
AVLTreeValue
avl_tree_lookup(AVLTree *tree, AVLTreeKey key)
{
    AVLTreeNode *node;
...
AVLTreeNode *
avl_tree_root_node(AVLTree *tree)
{
    return tree->root_node;
}

AVLTreeKey
avl_tree_node_key(AVLTreeNode *node)
{
    return node->key;
}

AVLTreeValue
avl_tree_node_value(AVLTreeNode *node)
{
    return node->value;
}

AVLTreeNode *
avl_tree_node_child(AVLTreeNode *node, AVLTreeNodeSide side)
{
...
AVLTreeNode *
avl_tree_node_parent(AVLTreeNode *node)
{
    return node->parent;
}

unsigned int
avl_tree_num_entries(AVLTree *tree)
{
    return tree->num_nodes;
}

static void
avl_tree_to_array_add_subtree(AVLTreeNode *subtree, AVLTreeValue *array, int *index)
{
    if (subtree == NULL) {
        return;
    }
...
AVLTreeValue *
avl_tree_to_array(AVLTree *tree)
{
    AVLTreeValue *array;
    int           index;
